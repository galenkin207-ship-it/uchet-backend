...
brigadesRouter.get("/", requireAuth, async (req, res) => {
...
brigadesRouter.post("/", requireAuth, async (req, res) => {
...
brigadesRouter.put("/:id", requireAuth, async (req, res) => {
  const { name, members = [] } = req.body || {};
  if (!name || !String(name).trim()) return res.status(400).json({ error: "name is required" });
  const { rows } = await pool.query(
...
brigadesRouter.delete("/:id", requireAuth, async (req, res) => {
  const { rows } = await pool.query(
